...
MSRunSim <- function(Digested, parameters) {
  ...
  for (i in seq_along(parameters$quant_colnames)) {
    
    name <- parameters$quant_colnames[i]
  ...
  allVals <- as.vector(unlist(MSRun[,parameters$quant_colnames]))
  # par(mfrow=c(1,2))
  # hist(allVals,100)
  ...
  # hist(allVals,100)
  # par(mfrow=c(1,1))
  MSRun[,parameters$quant_colnames] <- allVals
  
  # shuffle identifications
  shuffle <- sample(1:nrow(MSRun), parameters$WrongIDs*nrow(MSRun))
  for (i in 1:floor(length(shuffle)/2)) {
    temp <-  MSRun[shuffle[(i-1)*2+1], parameters$quant_colnames]
    MSRun[shuffle[(i-1)*2+1], parameters$quant_colnames] <- MSRun[shuffle[i*2],parameters$quant_colnames]
    MSRun[shuffle[i*2],parameters$quant_colnames] <- temp
  }
  if (length(shuffle)%%2 == 1) {
    temp <-  MSRun[shuffle[1], parameters$quant_colnames]
    MSRun[shuffle[1], parameters$quant_colnames] <- MSRun[shuffle[length(shuffle)],parameters$quant_colnames]
    MSRun[shuffle[length(shuffle)],parameters$quant_colnames] <- temp
  }
  ...
  if (parameters$FracModProt > 0) {
    for (mod in parameters$PTMTypes) {
      ...
      tmpModPosCount <- str_locate_all(modified$PepSequence, paste0(parameters$ModifiableResidues[[mod]],collapse="|"))
      ...
      if (NumForMisLoc > 0) {
        for (ind in sample(isModified[ModifiableCount > ModCount], NumForMisLoc)) {
          curr_pep <- MSRun[ind,]
          available_pos <- str_locate_all(curr_pep$PepSequence, paste0(parameters$ModifiableResidues[[mod]],collapse="|"))[[1]][,1]
