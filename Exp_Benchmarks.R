    ...
    # Reading MaxQuantfiles
    readMaxQuant <- function(folder="./", Param=NULL) {
      allPeps <- read.csv(paste0(folder,"/modificationSpecificPeptides.txt"), sep="\t")
      Prots <- read.csv(paste0(folder,"/proteinGroups.txt"), sep="\t")
      ...
      #TODO: add column for miscleavages in proteins
    ...
    MQout <- readMaxQuant("/tmp/FASP_10ug_1H_MaxquantResults/")
