...
paramGroundTruth <- list(#####   Ground truth generation
  ...
  "PTMTypes" = "ox",
  "PTMTypesDist" = c(1),
  "PTMTypesMass" = 15.9949,
  "PTMMultipleLambda" = 0.5,
  "ModifiableResidues" = list(c("M")),
  "ModifiableResiduesDistr" = list(c(1)),
  "RemoveNonModFormFrac" = 0
)
paramProteoformAb <- list(
  "QuantNoise" = c(0.25,0.5),
  #"QuantNoise" = 0.2,0.7
  "DiffRegFrac" = c(0.1),
  "DiffRegMax" = c(1.5),
...
paramDigest <- list(
  ##### Digestion
  "Enzyme" = "trypsin",
  "PropMissedCleavages" = 0.02, 
  "MaxNumMissedCleavages" = 4,
  "PepMinLength" = 7,
  "PepMaxLength" = 30,
  "LeastAbundantLoss" = c(0.0, 0.1),
  "EnrichmentLoss" = 0.0,
...
paramMSRun <- list(
  ##### MSRun
  "PercDetectedPep" = c(0.2,0.3,0.4,0.5),
  "PercDetectedVal" = c(0.05,0.1,0.2),
  "WeightDetectVal" = c(10, 100, 1000),
  "MSNoise" = c(0.25),
  "WrongIDs" = c(0.01),
...
par(mfrow=c(1,1))
save(BenchMatrix, file="BenchMatrix.RData")
...
# a <- list.files(resultFilePath)
# a <- sort(a, decreasing = T)
# 
# for (filename in a) {
#   if(file.info(paste0(resultFilePath,"/",filename))$size > 0) { 
#   load(paste0(resultFilePath,"/",filename))
# if (!is.null(Param$WrongIdentifications))
#   file.remove(paste0(resultFilePath,"/",filename))
#   } else {
#     file.remove(paste0(resultFilePath,"/",filename))
#     
#   }
