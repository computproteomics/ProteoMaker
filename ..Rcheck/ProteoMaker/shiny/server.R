...
server <- function(input, output, session) {
  ...
  output$parameters_valid <- renderText(ifelse(all_valid(), "All parameters are valid",
                                               "Please correct parameter values"))
  ...
  validate_param_values <- function(param_name, param_info, input_value) {
      ...
      if (!all(is.null(unlist(input_value))) && !all(is.na(unlist(input_value)))) {
        ...
        if ("datapath" %in% names(input_value)) {
          if (file.exists(input_value$datapath)) {
            ...
            file.copy(input_value$datapath, new_file_name, overwrite = TRUE)
    ...
    if (valid) {
      list(valid = TRUE, message = paste(param_name, "is valid."))
    } else {
      list(valid = FALSE, message = error_message)
    }
  }
  ...
  validate_ptm_params <- function(params, up_params) {
    ...
    # return validation result
    if (valid) {
      list(valid = TRUE, message = paste("PTM parameters ares valid."))
    } else {
      list(valid = FALSE, message = error_message)
    }
  }
  ...
  observeEvent(input$add_ptm, {
    ...
    if (length(ptm_aa) > 0) {
      ...
      params$PTMTypesMass$value[[1]][[ptm_name]] <-
        params$PTMTypesMass$choices[[which(params$PTMTypes$choices == ptm_name)]]
      ...
      update_ptm_info <- paste0(ptm_info(), ifelse(is.null(ptm_info()), "", "<br/>"),
        paste0("PTM: ", ptm_name,
               ...
               paste0(sapply(seq_along(ptm_aa), function(i) {
                 paste(" ", ptm_aa[[i]], ": ", ptm_aa_freq[[i]])
                 }),
                      collapse = ",")
               )
      )
