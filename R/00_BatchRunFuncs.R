...
#' data if available.
#'
#' When \code{Param} is supplied in the nested format returned by \code{def_param}
#' (i.e. it contains the sub-lists \code{paramGroundTruth}, \code{paramProteoformAb},
#' \code{paramDigest}, \code{paramMSRun}, and \code{paramDataAnalysis}), all
#' parameter combinations are enumerated and results for every combination are
#' returned as a named list keyed by the MD5 hash of each combination.  This
#' mirrors the parameter-expansion behaviour of \code{run_sims}.
#'
#' @param Param A list of parameters used in the simulation. Either the nested
#'   format produced by \code{def_param} (containing \code{paramGroundTruth},
#'   \code{paramProteoformAb}, \code{paramDigest}, \code{paramMSRun}, and
#'   \code{paramDataAnalysis} sub-lists, each of which may contain vectors of
#'   values to expand over), or a flat parameter list as stored by
#'   \code{run_sims} (e.g. \code{benchmarks[[1]]$Param}).
...
#'
#' @return When \code{Param} is a flat list: a list containing the loaded data
#'   objects (\code{Stats}, \code{StatsPep}, \code{Benchmarks}, etc.) for the
#'   requested stage, or \code{NULL} if no matching file is found.
#'   When \code{Param} is in the nested format: a named list of such result
#'   lists, one entry per parameter combination, keyed by the MD5 hash of each
#'   combination.  Combinations for which no saved file exists are silently
#'   omitted.
#'
#' @importFrom digest digest
#'
...
#'
#' # Retrieve all combinations at once using the nested Parameters format
#' Param <- def_param()
#' Param$paramGroundTruth$NumReps <- 5
...
get_simulation <- function(Param, Config, stage = "DataAnalysis") {
  ...
  # When Param is in the nested def_param() format, enumerate all combinations
  # and retrieve each one individually.
  if (all(c("paramGroundTruth", "paramProteoformAb", "paramDigest", "paramMSRun", "paramDataAnalysis") %in% names(Param))) {
    Param$paramGroundTruth$ProteoMakerVersion <- packageVersion("ProteoMaker")

    listtogroundtruth <- generate_combinations(Param$paramGroundTruth)
    listtoproteoformab <- generate_combinations(Param$paramProteoformAb)
    listtodigestion <- generate_combinations(Param$paramDigest)
    listtomsrun <- generate_combinations(Param$paramMSRun)
    listtodatanalysis <- generate_combinations(Param$paramDataAnalysis)

    allResults <- list()

    for (hh in seq_len(length(listtogroundtruth))) {
      gtParam <- lapply(listtogroundtruth[[hh]], function(x) x)
      if (stage == "GroundTruth") {
        result <- get_simulation(gtParam, Config, stage = "GroundTruth")
        if (!is.null(result)) {
          md5 <- digest::digest(as.list(gtParam), algo = "md5")
          allResults[[md5]] <- result
        }
        next
      }
      for (ii in seq_len(length(listtoproteoformab))) {
        pfabParam <- c(gtParam, listtoproteoformab[[ii]])
        pfParam <- c(
          pfabParam,
          list(QuantColnames = paste0(
            "C_",
            rep(1:pfabParam$NumCond, each = pfabParam$NumReps),
            "_R_",
            rep(1:pfabParam$NumReps, pfabParam$NumCond)
          ))
        )
        if (stage == "ProteoformAb") {
          result <- get_simulation(pfabParam, Config, stage = "ProteoformAb")
          if (!is.null(result)) {
            md5 <- digest::digest(as.list(pfabParam), algo = "md5")
            allResults[[md5]] <- result
          }
          next
        }
        for (jj in seq_len(length(listtodigestion))) {
          dgParam <- c(pfParam, listtodigestion[[jj]])
          if (stage == "Digest") {
            result <- get_simulation(dgParam, Config, stage = "Digest")
            if (!is.null(result)) {
              md5 <- digest::digest(as.list(dgParam), algo = "md5")
              allResults[[md5]] <- result
            }
            next
          }
          for (kk in seq_len(length(listtomsrun))) {
            msParam <- c(dgParam, listtomsrun[[kk]])
            if (stage == "MSRun") {
              result <- get_simulation(msParam, Config, stage = "MSRun")
              if (!is.null(result)) {
                md5 <- digest::digest(as.list(msParam), algo = "md5")
                allResults[[md5]] <- result
              }
              next
            }
            for (ll in seq_len(length(listtodatanalysis))) {
              tParam <- c(msParam, listtodatanalysis[[ll]])
              result <- get_simulation(tParam, Config, stage = "DataAnalysis")
              if (!is.null(result)) {
  ...
  # --- single-combination (flat Param) path ---
  ...
  for (i in names(titles)) {
    ...
    if (length(ref_par) == 1) {
      ...
      if (all(is.finite(range(benchmatrix[, i], na.rm = T)))) {
        ...
        names(xf) <- x
        # If compare_par is provided, show subsets in different colors with CI bands.
        ...
        if (!is.null(compare_par)) {
          ...
          pch_comp <- rep(pch.vals, length.out = compare_len)
          x_base <- unique(benchmatrix[, ref_par])
          x_base_plot <- if (is.numeric(x_base)) as.numeric(x_base) else seq_along(x_base)
          if (compare_len == 1) {
            subset_cols <- col
          } else {
            subset_cols <- colorspace::lighten(col, amount = seq(0, 0.4, length.out = compare_len))
          }
          # Base plot
          plot(
            x_base_plot, rep(NA, length(x_base_plot)),
            type = "n", xaxt = "n",
            xlab = params, ylab = i, ylim = myrange
          )
          axis(1, at = x_base_plot, labels = x_base, las = 1)
          title(
            main = paste0(titles[i]),
            col.main = col, font.main = 2
          )
          # Plot each subset
          for (j in seq_len(compare_len)) {
            sel <- benchmatrix[, compare_par] == compare_values[j]
            y_raw <- benchmatrix[sel, i]
            ...
            if (errorbar) {
              ...
              x_idx <- match(names(y_mean), as.character(x_base))
              x_plot <- x_base_plot[x_idx]
              ord <- order(x_plot)
              ok <- is.finite(y_mean) & is.finite(y_sd) & is.finite(x_plot)
              # CI band
              if (sum(ok) >= 2) {
                polygon(
                  c(x_plot[ord][ok[ord]], rev(x_plot[ord][ok[ord]])),
                  c((y_mean - y_sd)[ord][ok[ord]], rev((y_mean + y_sd)[ord][ok[ord]])),
                  col = grDevices::adjustcolor(subset_cols[j], alpha.f = 0.2),
                  border = subset_cols[j], lty = j, lwd = 0.6
                )
              }
              # Always draw per-point CI as vertical segments when sd is available
              seg_ok <- ok
              if (any(seg_ok)) {
                segments(
                  x_plot[seg_ok], y_mean[seg_ok] - y_sd[seg_ok],
                  x_plot[seg_ok], y_mean[seg_ok] + y_sd[seg_ok],
                  col = grDevices::adjustcolor(subset_cols[j], alpha.f = 0.6),
                  lwd = 1
                )
              }
              lines(x_plot[ord], y_mean[ord], col = subset_cols[j], lty = j)
              points(x_plot, y_mean, pch = pch_comp[j], col = subset_cols[j])
            } else {
              x_idx <- match(as.character(x_raw), as.character(x_base))
              x_plot <- x_base_plot[x_idx]
              points(x_plot, y_raw, pch = pch_comp[j], col = subset_cols[j])
            }
          }
        } else {
          y <- benchmatrix[, i]
      ...
      # Legend for compare_par case (colors)
      if (!is.null(compare_par) && identical(i, names(titles)[1])) {
        legend("topright", legend = compare_values, title = compare_par,
               col = subset_cols, pch = pch_comp,
               cex = 0.7, pt.cex = 0.7, x.intersp = 0.6, y.intersp = 0.7,
               bg = grDevices::adjustcolor("white", alpha.f = 0),
               box.col = NA, bty = "o")
      }
    } else {
