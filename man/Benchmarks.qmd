...
-   **AUC of ROC Curve for Correct Differentially Regulated Peptides** (`aucDiffRegPeptides.FDR_limma 2 vs 1.AUC`, label: *Peptide AUC (truth vs limma FDR)*): Area under the ROC curve for identifying truly regulated peptides using the limma-based FDR estimates.
...
-   **Number of Quantified Protein Groups** (`numQuantProtGroups`, label: **#Quantified proteins**): Count of protein groups that passed summarisation (respecting the configured shared/modified peptide settings) and retain at least one quantified value.
...
-   **TPR (True Positive Rate)** (`tprProt0.01.FDR_PolySTest 2 vs 1.TPR`, `tprProt0.05.FDR_PolySTest 2 vs 1.TPR`, label: *TPR protein (FDR \< 0.01/0.05)*): True positive rate for regulated protein groups at PolySTest-estimated FDR thresholds of 0.01 and 0.05.

-   **True FDR for Estimated FDR** (`tFDRProt0.01.FDR_PolySTest 2 vs 1.tFDR`, `tFDRProt0.05.FDR_PolySTest 2 vs 1.tFDR`, label: *True FDR (proteins)*): Actual false-discovery rate realised at those PolySTest thresholds.
...
-   **Proportion of Proteins with Miscleaved Peptides** (`propMisCleavedProts`, label: **% Miscleaved proteins**): Fraction of quantified protein groups that contain at least one peptide with a missed cleavage.

-   **Proportion of Regulated Proteins with Wrong Identified Peptides** (`propDiffRegWrongIDProt0.01.FDR_PolySTest 2 vs 1`, `propDiffRegWrongIDProt0.05.FDR_PolySTest 2 vs 1`, label: **% Wrong-ID (proteins)**): Fraction of regulated proteins with wrongly identified peptides at 0.01 and 0.05 PolySTest FDR thresholds.
...
-   **AUC for Correctly Regulated Modified Peptides** (`aucDiffRegAdjModPep.FDR_limma 2 vs 1.AUC`, label: *AUC (adj. mod. peptides)*): AUC calculated after adjustment for parent protein abundance (reported only when at least 200 modified peptides have matching protein quantification).

-   **TPR** (`tprAdjModPep0.01.FDR_limma 2 vs 1.TPR`, `tprAdjModPep0.05.FDR_limma 2 vs 1.TPR`, label: *TPR (adj. mod. peptides)*): True positive rate for regulated modified peptides at estimated FDRs 0.01 and 0.05.

-   **True FDR** (`tFDRAdjModPep0.01.FDR_limma 2 vs 1.tFDR`, `tFDRAdjModPep0.05.FDR_limma 2 vs 1.tFDR`, label: *True FDR (mod. peptides)*): Actual FDR of modified peptide results at FDRs 0.01 and 0.05.

-   **Share of Significant Modified Peptides** (`propDiffRegPepWrong0.01.FDR_PolySTest 2 vs 1`, `propDiffRegPepWrong0.05.FDR_PolySTest 2 vs 1`, label: **% Wrongly significant (mod. peptides)**): Fraction of modified peptides whose FDR values fall below 0.01 or 0.05, respectively.
...
| AUC of ROC (Peptides) | aucDiffRegPeptides.FDR_limma 2 vs 1.AUC | Peptide AUC (truth vs limma FDR) | Peptide |
...
| AUC (Adj. Modified Peptides) | aucDiffRegAdjModPep.FDR_limma 2 vs 1.AUC | AUC (adj. mod. peptides) | PTM |
| TPR (Adj. Mod Peptides) at 0.01 | tprAdjModPep0.01.FDR_limma 2 vs 1.TPR | TPR (adj. mod. peptides, 0.01) | PTM |
| TPR (Adj. Mod Peptides) at 0.05 | tprAdjModPep0.05.FDR_limma 2 vs 1.TPR | TPR (adj. mod. peptides, 0.05) | PTM |
| True FDR (Adj. Mod Peptides) at 0.01 | tFDRAdjModPep0.01.FDR_limma 2 vs 1.tFDR | True FDR (mod. peptides, 0.01) | PTM |
| True FDR (Adj. Mod Peptides) at 0.05 | tFDRAdjModPep0.05.FDR_limma 2 vs 1.tFDR | True FDR (mod. peptides, 0.05) | PTM |
