...
*Param\$PropModPerProt*: Multiplier for generating additional modified proteoforms per selected protein. Setting to `1` (or the default `0`) keeps exactly one modified proteoform per protein. Values `>= 2` create extra modified copies (e.g., `2` yields two independently modified proteoforms per protein). To obtain **2 proteoforms per protein** (one modified + one unmodified), set `PropModPerProt = 1` and `RemoveNonModFormFrac = 0` (see below). To obtain **2 modified proteoforms** and no unmodified form, set `PropModPerProt = 2` and `RemoveNonModFormFrac = 1`.
...
*Param\$PTMMultipleLambda*: Controls the average number of modification sites per proteoform via a truncated Poisson distribution scaled by the number of modifiable sites in the sequence. Setting `PTMMultipleLambda = 0` produces **exactly one modified site per PTM type per proteoform**: a single site is chosen from all modifiable residues for that PTM type, weighted by the background residue frequency distribution. For small positive values (e.g., `0.1`) the expected number of modifications increases gradually. When using a **single PTM type**, `PTMMultipleLambda = 0` therefore guarantees **exactly one modified site per proteoform** overall. Defaults to `0`.
...
**Recipe — max one modified site per proteoform, two proteoforms per protein:**
...
Param$paramGroundTruth$PTMMultipleLambda  <- 0   # at most 1 modified site per proteoform
