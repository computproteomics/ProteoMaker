...
*Param\$PropModPerProt*: Multiplier for generating additional modified proteoforms per selected protein. Values of `0` or `1` (and the default `0`) keep exactly one modified proteoform per protein — the difference has no effect since the duplication step only activates for values `>= 2`. Values `>= 2` create that many independently modified proteoforms per protein (e.g., `2` yields two modified proteoforms). To obtain **2 proteoforms per protein** (one modified + one unmodified), set `PropModPerProt = 1` and `RemoveNonModFormFrac = 0` (see below). To obtain **2 modified proteoforms** and no unmodified form, set `PropModPerProt = 2` and `RemoveNonModFormFrac = 1`.
...
*Param\$PTMMultipleLambda*: Controls the average number of modification sites per proteoform via a truncated Poisson distribution (truncated to [1, N] where N is the number of modifiable sites) scaled by the number of modifiable sites in the sequence. Must be positive to produce modifications for proteins with more than one modifiable residue. Use a small value (e.g., `0.001`) to obtain on average approximately one modified site per proteoform per PTM type; larger values increase the expected number of modifications. Defaults to `0`, which disables multi-site sampling — provide a positive value when PTMs are enabled.
...
Param$paramGroundTruth$PTMMultipleLambda  <- 0.001 # small lambda -> ~1 modified site per proteoform
